...
export default function ModelEnhancementAlert({ migrationId }) {
  ...
  const checkEnhancements = async () => {
    try {
      if (api.getModelEnhancements) {
        const response = await api.getModelEnhancements(migrationId);
        setHasEnhancements(response.has_enhancements);
        setEnhancementInfo(response);
      } else {
        setHasEnhancements(false);
      }
    } catch (error) {
  ...
  const handleDownloadGuide = async () => {
    try {
      if (api.downloadEnhancementGuide) {
        await api.downloadEnhancementGuide(migrationId);
  ...
  const handleDownloadAll = async () => {
    try {
      if (api.downloadAllEnhancements) {
        await api.downloadAllEnhancements(migrationId);
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-amber-800 mb-4">
            Some calculations use <strong>ThoughtSpot Window/Moving Functions</strong> (moving_average, cumulative, etc.)
