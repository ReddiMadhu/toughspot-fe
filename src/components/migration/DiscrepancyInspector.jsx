    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-emerald-50 rounded-lg p-3">
            ...
            <div className="text-2xl font-bold text-emerald-700">{passedSlices.length}</div>
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
          ...
          <p className="text-xs text-gray-600 mt-1">Comparing ThoughtSpot ground truth vs. DAX execution</p>
        </div>
              ...
              {test_slices.map((slice, index) => (
                <tr key={index} className={slice.passed ? 'bg-white' : 'bg-red-50'}>
                  ...
                  <td className="px-4 py-3 text-sm text-right font-mono text-gray-900">
                    {slice.source_value?.toLocaleString(undefined, { maximumFractionDigits: 2 }) ?? slice.tableau_value?.toLocaleString(undefined, { maximumFractionDigits: 2 }) ?? 'NULL'}
                  </td>
                  <td className="px-4 py-3 text-sm text-right font-mono">
                    <span className={slice.passed ? 'text-emerald-700' : 'text-red-700'}>
                  ...
                  <td className="px-4 py-3 text-sm text-right">
                    {slice.passed ? (
                      ...
                      <div className="text-right">
                        <div className="text-red-700 font-medium font-mono">
        ...
        <div className="w-full bg-gray-200 rounded-full h-3">
          <div
            className={`h-3 rounded-full transition-all duration-500 ${
              pass_rate === 1 ? 'bg-emerald-500' : pass_rate > 0.9 ? 'bg-amber-500' : 'bg-red-500'
            }`}
